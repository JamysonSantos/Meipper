...
if (photoInput) {
    ...
    async handleRegister(email, password, displayName, photoFile) {
  try {
    // Cria usuário
    ...
    if (photoFile) {
      const storageRef = window.ref(
        ...
        `profilePhotos/${user.uid}/avatar_${Date.now()}`
      );
    ...
    await window.setDoc(
      ...
      {
        email: user.email,
        nome: displayName || "Usuário",   // 🔑 agora usa "nome"
        photoURL: photoURL || null,       // 🔑 salva a foto
